...
for addr in ${SERVERS}; do
    addr=${USER_NAME}@${addr}
    ssh ${addr} "sudo echo 'successful sudo'"
    scp ./scripts/* ${addr}:~/
done
# check directory
...
if [ "${set_dir}" = "y" ]; then
    for addr in ${SERVERS}; do
        ...
        ssh ${addr} "mount.sh ${device} /${name} && sudo ln -s /${name} /data\
    ...
    for addr in ${SERVERS}; do
        addr=${USER_NAME}@${addr}
        OWNER=$(ssh ${addr} "ls -ld ${DATA_DIR}" | awk '{print $3}')
        if [ ${OWNER} != ${USER_NAME} ]; then
            echo "DATA_DIR owner isn't ${USER_NAME}" 
        fi
        OWNER=$(ssh ${addr} "ls -ld ${BACKUP_DIR}" | awk '{print $3}')
        if [ ${OWNER} != ${USER_NAME} ]; then
